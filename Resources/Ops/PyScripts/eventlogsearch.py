...
def getmostrecentrecordnum(eventlog='security'):
    ...
    return eventobject.eventlog[0].mostrecentrecordnum
    ...
    if (len(eventobject.record) > 0):
        ...
        for record in eventobject.record:
            rec_dict = {
                ...
                'id': record.id,
                'datewritten': record.datewritten,
...
def eventlogtime(log='Security', max=100, num=1000, id_list=None, sid=None, string_opt_list=None, startrecord=None, xpath=None, target=None, summary=False, logons=False):
    ...
    eventcmd.max = max
    eventcmd.num = num
    id_list = id_list.split(',') if (id_list is not None) else [None]
    if (sid is not None):
        eventcmd.sid = sid
    ...
    for record in record_list:
        ...
        elif (last_date == (this_date - timedelta(days=1))):
...
def main(args):
    ...
    while True:
        if options.monitor:
            ...
            querymax = querynum
            if (querymax == 0):
                dsz.ui.Echo(f'[{ops.timestamp()}] No new records', dsz.WARNING)
